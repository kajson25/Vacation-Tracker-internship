...
@Component
class Mapper {
    fun mapEmployeeToResponse(employee: Employee): EmployeeResponseDTO = EmployeeResponseDTO(employeeId = employee.employee_id, email = employee.email)
    ...
    fun mapUsedDays(usedDays: UsedDays): UsedDaysDTO? =
        usedDays.employee?.let { usedDays.beginDate?.let { it1 -> usedDays.endDate?.let { it2 -> UsedDaysDTO(beginDate = it1.toLocalDate(), endDate = it2.toLocalDate(), employeeId = it.employee_id) } } }
